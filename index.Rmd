...
- Part 2: <a href = "#/3"> Probability & distributions </a>
- Part 3: <a href = "#/4"> Data manipulation </a>
- Part 4: <a href = "#/5"> Looping and vectorization </a>
- Part 5: <a href = "#/6"> Functions </a>
- Part 6: <a href = "#/7"> Data visualization using ggplot2 </a>
...
- But, it can be a good idea to follow conventions -- this makes your code more readable to others
    ...
    - <a href = "https://style.tidyverse.org/" target = "_blank">Tidyverse Style Guide</a>
...
- There are many, many R packages - currently 23,897 (when I first taught this course in 2016, there were "only" 7,742)
...
<a href = "https://r-charts.com/colors/" target = "_blank">Colors in R</a>

<a href = "https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf" target = "_blank">R color cheatsheet</a>
...
Index individual elements of a data frame using square brackets `df[row#, column#]`
...
- Or you can index a column by name using the dollar sign (`$`) operator
...
<a href = "https://adv-r.hadley.nz/vectors-chap.html" target = "_blank">Advanced R - Data structures (Vectors)</a>
...
## For-loops - brackets, parentheses, braces
...
3. Find the 4th element in the output vector from question 1 and the output list from question 2.
...
First, create a vector, `x`, with all integers from 1 to 100.
...
- The `apply` family of functions allows you to repeatedly do a function to every row, column, or element in a data frame (or vector, or list)
- `apply(X, MARGIN, FUN)` function takes three arguments
    - `X`: your matrix or data frame
...
Use `apply()` to take the median of the columns of matrix `M`
...
<a href = "https://adv-r.hadley.nz/subsetting.html" target = "_blank">Advanced R - Subsetting</a>

<a href = "https://adv-r.hadley.nz/control-flow.html" target = "_blank">Advanced R - Control flow</a>
...
<a href = "https://adv-r.hadley.nz/functions.html" target = "_blank">Advanced R - Functions</a>
...
state.fig <- ggplot(data = state.df, aes(x = Income, y = Life.Exp))
...
Increase the point size using the `size` argument in `geom_point()`

```{r, fig.height = 3.25, fig.width = 4.5}
ggplot(data = state.df, aes(x = Income, y = Life.Exp)) + 
    geom_point(size = 3)
```
...
ggplot(data = state.df, aes(x = Population, y = Murder, color = state.region)) + 
    geom_point(size = 2)
```
...
ggplot(data = diamonds.samp, aes(x = carat, y = price, color = color)) + geom_point()
```
...
ggplot(faithful, aes(waiting)) + geom_density(fill = "paleturquoise")
```
...
- Specify the dimensions (`height`, `width`) and resolution (`dpi`, dots per inch)
