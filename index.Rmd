...
This course was developed by Ben Weinstein (2014), Mike McCann (2015), and Nicole Kinlock (2016, 2017) as a workshop in the Department of Ecology & Evolution at Stony Brook University
...
## Your turn {data-background-color=#4387BA}
...
- <b>Use hashtags (`#`) to comment on your script.</b> Anything to the right of a `#` is ignored. Good scripts have comments before every major block of code.
...
- You can use the `seq(from = , to = , by = )` function to create a sequential vector with a particular minimum (`from`), maximum (`to`), and increment (`by`)
...
## Data structures: 1. Vectors - Index by position
...
## Data structures: 1. Vectors - Index by name
...
- A <b>function</b> is an object that performs a task given some inputs (called <i>arguments</i>). R has many functions already available, but you can also write your own functions.
- Try using the tab key (`↹`) while entering arguments in any function to discover a useful feature of RStudio
- Functions are called in the format: `NameOfFunction(arguments)`   
- The output of a function can be saved as an object: `output <- NameOfFunction(arguments)`
...
- There are lots of packages commonly used by ecologists and evolutionary biologists (`ape`, `sf`, `vegan`, `lme4`, `dplyr`, ...)
...
- Additionally, you have to <b>load</b> packages in order to use them every time you restart R. This lets R know which packages to load and not waste memory by loading all possible functions.    
...
- It is useful to generate a sample from a statistical distribution
- To generate a sample of 1000 elements from a standard normal distribution (mean 0 and standard deviation 1) we can use the `rnorm()` function
...
1. Draw 100 random numbers from a normal distribution with a mean of 3 and a standard deviation of 2, and assign it to an object `a`.
...
- For example, the `rpois()` function is the random sampler for the Poisson distribution and it has only one parameter, <i>lambda</i> ($\lambda$).
...
- Barplots are a basic type of univariate plot
- The R function to create one is `barplot()`
...
- Histograms are also a common univariate plot
- Histograms place data into "bins", and count the number (frequency) of data falling into each bin
- Bins are usually plotted as bars, with bin range on the x axis and frequency on the y axis
...
sample <- rnorm(1000)
hist(sample, col = "palevioletred", main = "", xlab = "Value", ylab = "Frequency")
```
...
- You can add straight lines to a plot using the `abline()` function
- Argument `a` specifies the intercept and `b` specifies the slope
...
- `lm()` fits a linear model of <b>y</b> (`sample2`) as a function of <b>x</b> (`sample1`).
...
You can find more on plotting in Part 6.

---

## Practice {data-background-color=#BA43B0}
...
## Data str.: 2. Data frames - Useful functions
...
- `str()` - structure of object <i>(very useful!)</i>
...
## Data structures: 3. Factors

- Diversion!
- In the `iris` data frame, species are encoded as a <b>factor</b>
- Factors, like vectors, are one dimensional
- Factors describe categorical data, ordered by levels
...
- To make a vector into a factor, use the `factor()` function
- Provide the levels (using the `levels` argument) as a vector in your desired order

```{r}
exp.df <- data.frame(Individual = 1:4, Treatment = c("Drought", "Control", "Drought", "Control"))
str(exp.df$Treatment)
exp.df$Treatment <- factor(x = exp.df$Treatment, levels = c("Drought", "Control"))
str(exp.df$Treatment)
```

---

## Data str.: 2. Data frames - Subsetting
...
## Data str.: 2. Data frames - Index by position
...
## Data str.: 2. Data frames - Index by name
...
5. What happens when you use negative numbers to index the `iris` dataframe? <i>Hint:</i> Use the function `dim()` on the original and final objects.

---

## Data str.: 2. Data frames - Logical indexing

- It can be helpful to subset your data using logic rather than by position/name
- Let's subset the `iris` data frame to petal widths >1 cm

```{r}
# create logical vector based on logical expression
large.petals.logi <- iris$Petal.Width > 1  
head(large.petals.logi)
# subset iris df using logical vector
...
Note that quotation marks are needed when subsetting by characters or factors
...
- It's also possible to combine logical statements:
  - Use <b>`&`</b> (AND) if you want <u>both</u> statements to be true.  
  - Use <b>`|`</b> (OR) if you want <u>either</u> statement to be true.
...
4. What happens when you add a `!` before a logical statement? <i>Hint:</i> Compare `iris[iris$Species == "setosa", ]` and `iris[!(iris$Species == "setosa"), ]`.

---

## Data str.: 2. Data frames - Building from scratch
...
## Data str.: 2. Data frames - Add columns
...
## Data str.: 2. Data frames - Remove columns
...
## Data structures: 4. Matrices

- Similar to data frames, matrices are two-dimensional
- However, they can consist only of numbers (not characters or factors).
...
- Before we import the example CSV file, we will need to consider <b>working directories</b>
...
- Load the downloaded seedling data to your environment using the `read.csv()` function
...
- Loops are an important programming tool across languages 
- The first loop we will learn is a <b>for loop</b>
...
## For loops - Our first loop (pseudocode)
...
- <b>Be sure to distinguish among</b>
    - curly braces `{}` 
    - parentheses `()`  
    - square brackets `[]`
...
- Square brackets `[]` are used to access <i>elements</i> of vectors, matrices, and data frames
- The number of dimensions is very important.

```{r}
test.vec <- seq(0.1, 10, 0.1)
...
- The loop variable, `i`,  is not directly called in this loop, unlike in the previous examples
- The operation `x <- x^2` is done four times
- `x` changes each iteration because it is re-assigned in the loop
...
4. Create a for loop that divides all even integers from 0 to 20 by 10. <i>Hint</i>: Use the `seq()` function.
...
- It is more common to use the loop variable to loop over elements of a vector
...
## Data structures: 5. Lists 

- <b>Lists</b> are another of the five basic data structures in R
- Lists, like vectors, are one dimensional
- First, let's compare lists with vectors -- because the elements of a vector must all be the same type, in this example, `1` and `2` are converted to characters because `"blue"` is included.
...
## Data structures: 5. Lists - Constructing lists
...
## Data structures: 5. Lists - List indexing
...
- Let's use an <b>if</b> statement inside a for loop
...
- The <i>apply</i> family of functions allows you to process whole rows, columns, or list elements
- They can be used to replace for loops
...
1. Create a function that takes in two arguments, `x` and `y`, and computes `x * y * 2`.
2. Create a function that takes in three arguments, and makes a vector from the result.
3. Create a function that counts the number of matching elements in two separate vectors. <i>Hint:</i> use `%in%` to create a logical statement.
...
You can increase the size of points using the `size` argument in `geom_point()`
...
You can differentiate state region by <b>color</b> using the `color` argument in `aes()`
...
You can also differentiate state region by <b>shape</b> using the `shape` argument in `aes()`
...
You can change some of the aesthetics, such as `binwidth` or `fill`
...
- The syntax is `facet_grid(Species ~ .)`, where rows are species and there is only one column
...
Flip to `facet_grid(. ~ Species)` in order to get one row and three columns
...
- Type `stat_` and hit the tab key (`↹`) to see all options
    - Then, use `?stat_` to see the help screen
...
- You can define the colors, labels, limits, etc. for your scale
...
- For example: 
    ...
    - `scale_*_manual()`: discrete data, you specify colors
...
ggplot(data = state.df, aes(x = Population, y = Area, color = state.region)) + 
    ...
    scale_color_manual(values = c("palegreen3","palevioletred3","peachpuff3", "paleturquoise3"))
```
...
ggplot(data = state.df, aes(x = Life.Exp, y = HS.Grad, color = state.region, shape = state.region)) +
    geom_point(size = 1) + 
    labs(x = "Life expectancy (in years)", y = "High school graduation rate (percent)") + 
    geom_text(aes(label = state.abb, size = 1.2, hjust = 0.1, vjust = 0.1))
```
...
- You can use <b>themes</b> to control the aesthetics of the figure
...
- You can also specify the dimensions and resolution
- Figures are saved in your current working directory unless you include the complete file path in the `filename` argument

```{r, fig.height = 3, fig.width = 6, eval = FALSE}
iris.fig <- ggplot(iris, aes(Species, Sepal.Length)) + 
    geom_bar(stat = "identity") + theme_bw()
ggsave(filename = "IrisFigure.png", iris.fig, dpi = 600, height = 4, width = 4,
       units = "in")
```
...
Your data structure must be a data frame in order to plot with ggplot2
...
- Data is much easier to analyze if it looks like this (<b>long</b> format), with only one row per observation
...
```{r, message = FALSE}
library(reshape2)
test.df.long <- melt(data = test.df.wide, variable.name = "Species",
                     value.name = "Mass")
head(test.df.long)
dim(test.df.long)
```
...
ggplot(test.df.long, aes(x = Mass)) + facet_grid(. ~ Species) + 
