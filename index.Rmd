...
- Barplots (`barplot()`) are a basic type of univariate plot

```{r, fig.height = 2, fig.width = 5}
head(rivers)
barplot(height = rivers[1:10], col = "paleturquoise", 
        xlab = "Rivers in North America", 
...
- Histograms (`hist()`) are another common univariate plot
...
Scatterplots are useful for showing the relationship between two variables (`plot(x, y)`)
...
plot(x = sample1, y = sample2, xlab = "Sample 1", 
     ylab = "Sample 2")
```
...
- `lm()` fits a linear model of **y** (`sample2`) as a function of **x** (`sample1`)
...
```{r}
# pre-loaded data frame in R - measurements of black cherry trees
head(trees)
```
...
- Quick diversion with a new data structure!
...
1. Return the 9th element of the `Sepal.Width` column
2. Subset the 15th row
...
- To subset the `iris` data frame to petal widths >1 cm:
...
- Load the `seedlings.csv` file using the `read.csv()` function
...
3. What is the difference in the mean height for *Rosa multiflora* seedlings and *Rubus occidentalis* seedlings?
...
write.csv(iris, file = "iris.csv", row.names = FALSE, fileEncoding = "UTF-8")
```
...
- Square brackets `[]` are used to access *elements* of vectors, matrices, and data frames
...
You can perform operations on the **loop variable**, `i`
...
4. Create a for loop that divides all even integers from 0 to 20 by 10. *Hint*: Use the `seq()` function.
...
- Loops exist in their own enclosed environment (in contrast to the **global environment**, or the main workspace)
...
- **Lists** are another of the five basic data structures in R
...
- Let's use an **if** statement inside a for loop
...
- **If** statements are often paired with **else** statements, used when we want to run specific code in a given case (`if`) and the same code for every other possible case (`else`)
...
- We can end a loop based on an **if** and **break** statement
...
- We can skip the current iteration of the loop using using an **if** and **next** statement
...
- The *apply* family of functions allows you to process whole rows, columns, or list elements
...
It is also possible to *define your own functions*. This is especially important if you are going to write the same lines of code over and over again. R functions are objects just like anything else.
...
3. Create a function that counts the number of matching elements in two separate vectors. *Hint:* use `%in%` to create a logical statement.
...
2. Write a function that calculates the mean of every column in a data frame. Code the function so that it does not evaluate the column mean if the column elements are not numbers, using the logical statement **`class(x) != "numeric"`**. Try your function on the `iris` dataset.
...
You can differentiate state region by **color** using the `color` argument in `aes()`
...
You can also differentiate state region by **shape** using the `shape` argument in `aes()`
...
- Plots can also have **facets**, which divide a plot into subplots based on a factor
...
- You can use **themes** to control the aesthetics of the figure
...
- Sometimes data looks like this, with multiple observations in each row (**wide** format)
...
- Data is much easier to analyze if it looks like this (**long** format), with only one row per observation
